...
import re
import logging
from .services.search_service import SearchService, HS_CODE_RE
...
class SearchViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        # If the user typed something like "1702.4" or "170230" directly in
        # the search bar (no explicit hs_code param), promote it so the fast
        # hs_code path is used in _resolve_subcategories.
        # ------------------------------------------------------------------
        if not hs_code and HS_CODE_RE.match(query.strip()):
            hs_code = query.strip()
    ...
    def autocomplete(self, request):
        ...
        for s in suggestions:
            ...
            result.append({
                ...
                "tx_count":     s["tx_count"],
                "match_type":   s.get("match_type", "name"),
            })

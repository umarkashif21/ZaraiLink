...
class SearchService:
    ...
    def execute_search(
        ...
        if nlu_result is not None:
            logger.info(f"[CACHE] NLU cache HIT for query='{raw_query[:40]}' — skipping LLM call")
        else:
            logger.info(f"[CACHE] NLU cache MISS for query='{raw_query[:40]}' — running full NLU")
            nlu_result = self._nlu_engine.parse(raw_query, ui_context=ui_context)
            cache.set(_cache_key, nlu_result, timeout=86400)  # 24 hours
        logger.info(f"[TIMING] NLU parse: {time.perf_counter() - t0:.3f}s")
        ...
        if not hs_code and variant_list:
            ...
            # Only lock in exact match when no other variant's name has the exact match
            # as a prefix — i.e. "Dextrose" should NOT lock in when "Dextrose Anhydrous"
            # and "Dextrose Monohydrate" also exist. In that case merge all.
            if exact:
                has_longer = any(
                    v['name'].lower().strip().startswith(kw_lower + " ")
                    for v in variant_list if v['id'] != exact[0]['id']
                )
                if not has_longer:
        ...
        is_broad = False
        if not hs_code and not subcat_ids and product_keyword:
            is_broad = True
    ...
    def _resolve_subcategories(
        ...
        if hs_code:
            ...
            # Try exact match first, then startswith (handles "1701" matching "1701.991" etc.)
            ...
            # Cascade: for deep codes like "1702.30.90" that don't exist in DB, progressively
            # shorten the code until a parent chapter matches ("1702.30" → "1702.3" → "1702").
            if not scs and '.' in hs_code:
                parts = hs_code.split('.')
                for i in range(len(parts) - 1, 0, -1):
                    prefix = '.'.join(parts[:i])
                    scs = list(ProductSubCategory.objects.filter(hs_code__startswith=prefix).order_by('name'))
                    if scs:
                        break
        ...
        # PASS 5: Edit-distance fallback for transposition/scramble typos that
        # produce zero trigram overlap (e.g. "suagr" vs "sugar", "cotten" vs "cotton").
        # Uses difflib which is stdlib — no extra dependency.
        # Guard: require >= 5 chars so short words ("urea", "corn") don't false-match
        # "sugar" (SequenceMatcher("urea","sugar") ≈ 0.67 which would pass a 0.6 threshold).
        if not sc_qs and not item_qs and product_keyword and len(product_keyword) >= 5:
            import difflib
            kw_lower = product_keyword.lower()
            all_scs   = list(ProductSubCategory.objects.select_related("category").all())
            all_items = list(ProductItem.objects.select_related("sub_category__category").all())

            def _best_ratio(name):
                # Compare against full name and each individual word
                ratios = [difflib.SequenceMatcher(None, kw_lower, name.lower()).ratio()]
                for word in name.lower().split():
                    if len(word) >= 3:
                        ratios.append(difflib.SequenceMatcher(None, kw_lower, word).ratio())
                return max(ratios)

            sc_scored  = sorted([(s, _best_ratio(s.name))  for s in all_scs],   key=lambda x: -x[1])
            it_scored  = sorted([(i, _best_ratio(i.name))  for i in all_items], key=lambda x: -x[1])

            EDIT_THRESHOLD = 0.75
            sc_qs  = [s for s, r in sc_scored[:5]  if r >= EDIT_THRESHOLD]
            item_qs = [i for i, r in it_scored[:5] if r >= EDIT_THRESHOLD]
